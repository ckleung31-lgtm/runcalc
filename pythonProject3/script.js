...
function buildWeek(days, paces, longRunKm, interval, goal, week, totalWeeks, pace10kSec) {
  let t = {
    rest: "休息", interval: "間歇跑", recoveryRun: "恢復跑", tempo: "節奏跑", easy: "輕鬆跑", long: "長課",
    crossTraining: "交叉訓練", dayMon: "星期一", dayTue: "星期二", dayWed: "星期三", dayThu: "星期四",
    dayFri: "星期五", daySat: "星期六", daySun: "星期日"
  };
  let weeksToRace = totalWeeks - week;
  ...
  let recoveryDetail = crossTrain ? `${t.crossTraining}: ${crossTrainDetail}` : `${recoveryMin}min 恢復跑 @ ${paces.recovery}`;
  let baseWeek = [
    ...
    { day: t.daySat, type: t.easy, detail: `${easyMin}min 輕鬆跑 @ ${paces.easy}` },
    { day: t.daySun, type: t.long, detail: (weeksToRace === 0) ? "🏁 RACE DAY" : `${longRunKm}km 長課 @ ${paces.long}` }
  ];
  ...
  if (days <= 2) {
    ...
    weekPlan[3] = { day: t.dayThu, type: t.easy, detail: `${easyMin}min 輕鬆跑 @ ${paces.easy}` };
    weekPlan[6] = { day: t.daySun, type: t.long, detail: `${Math.round(longRunKm * 0.7)}km 長課 @ ${paces.long}` };
    return weekPlan;
  }
  let keepTypes = {
  ...
  if (days === 6) return weekPlan.map(d => d.day === t.dayFri ? { day: d.day, type: t.easy, detail: `${easyMin}min 輕鬆跑 @ ${paces.easy}` } : d);
  return weekPlan;
}
